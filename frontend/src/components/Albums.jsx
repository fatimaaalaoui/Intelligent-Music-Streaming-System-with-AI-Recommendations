...
export default function AlbumsPage({ onPlayTrack }) {
  const { id } = useParams();
  ...
  // ✅ Normalisation UNIQUE (playlist album)
  const playlist = useMemo(() => {
    const coverFallback = album?.image || "";
    const artistFallback = album?.artistName || "Artiste Jamendo";

    return (Array.isArray(tracks) ? tracks : [])
      .map((t) => ({
        // ⚠️ IMPORTANT: évite audioUrl comme id pour que like/dislike marche
        id: t._id || t.id,
        title: t.title || t.name || "Titre",
        artist: t.artistName || t.artist || artistFallback,
        cover: coverFallback || t.image || t.albumImage || "",
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // 1) Tracks
        const resTracks = await fetch(`http://localhost:5000/api/albums/${id}/tracks`);
        if (!resTracks.ok) throw new Error("API tracks: " + resTracks.status);
        const tracksJson = await resTracks.json();

        // 2) Album (si pas dans state)
        let albumObj = albumFromState;
        if (!albumObj) {
          const resAlbumList = await fetch("http://localhost:5000/api/albums?limit=200");
          if (resAlbumList.ok) {
            const all = await resAlbumList.json();
            albumObj = all.find((a) => String(a._id) === String(id)) || null;
          }
        }

        if (cancelled) return;

        setTracks(Array.isArray(tracksJson) ? tracksJson : []);
        if (albumObj) setAlbum(albumObj);

        // ✅ IMPORTANT: on stocke DANS le contexte la version normalisée (player + sidebar)
        const normalized = (Array.isArray(tracksJson) ? tracksJson : [])
          .map((t) => ({
            id: t._id || t.id,
            title: t.title || t.name || "Titre",
            artist: t.artistName || t.artist || albumObj?.artistName || "Artiste Jamendo",
            cover: albumObj?.image || t.image || t.albumImage || "",
            src: t.audioUrl || t.src || "",
            duration: t.duration,
          }))
          .filter((x) => x.id && x.src);
  ...
  }, [id, albumFromState, setAlbumTracks]);

  const heroTitle = album?.title || "Album Jamendo";
  const heroArtist = album?.artistName || "Artiste Jamendo";
  const heroImage = album?.image;

  // ✅ PLAY ALBUM : 1ère track + playlist complète
...
function TracksTable({ tracks, playlist, onPlayTrack }) {
  const handlePlayTrack = (t) => {
    ...
    // ✅ track sélectionnée + playlist complète
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-white/60">
                {t.artistName || t.artist || "Artiste Jamendo"}
              </p>
            </div>
